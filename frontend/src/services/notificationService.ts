...
class NotificationService {
  ...
  /**
   * Envia uma notificação de pedido criado
   * No iOS, usa service worker quando disponível (requer PWA instalado)
   */
  async notifyOrderCreated(amount: number, orderNumber?: string): Promise<void> {
    console.log('[NotificationService] Tentando enviar notificação de pedido criado:', { amount, orderNumber });
    ...
    try {
      ...
      const notificationBody = `Você recebeu um pedido de ${formattedAmount}`;
      ...
      console.log('[NotificationService] Criando notificação de pedido criado:', {
        title: notificationTitle,
        ...notificationOptions
      });
      ...
      if ('serviceWorker' in navigator) {
        try {
          ...
          // No iOS, SEMPRE usar service worker
          await registration.showNotification(notificationTitle, notificationOptions);
          console.log('[NotificationService] ✅ Notificação de pedido criado enviada via Service Worker');
          return;
        } catch (swError) {
          console.error('[NotificationService] ❌ Erro ao usar Service Worker:', swError);
          // No iOS, se o service worker falhar, não tentar fallback
      ...
      console.log('[NotificationService] Notificação de pedido criado criada com sucesso');
      ...
      console.error('[NotificationService] Erro ao enviar notificação de pedido criado:', error);
    }
  }
